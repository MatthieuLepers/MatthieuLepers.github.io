function DnaRedBullet(ship, id, sign)
{
	...
	this.img = ship.game.textures.projectile_red_dna_bullet.getPath();
	this.width = ship.game.textures.projectile_red_dna_bullet.getWidth();
	this.height = ship.game.textures.projectile_red_dna_bullet.getHeight();
	
	if (sign > 0)
	{
		if (this.id.contains('bitmoduleredshot'))
		{
			this.top = ship.top - 31;
...
/* ----- Events ----- */
DnaRedBullet.prototype.addEventListener = function(eventName, action)
{
	this.listeners.push(new Array(eventName, new EventListener(action)));
}

DnaRedBullet.prototype.removeEventListener = function(eventName, action)
{
...
DnaRedBullet.prototype.fire = function(event)
{
	event.dispatchEvent();
}

DnaRedBullet.prototype.onLaunch = function()
{
...
DnaRedBullet.prototype.destroy = function()
{
...
/* ----- Animation ----- */
DnaRedBullet.prototype.anim = function(params)
{
	var bullet = params[0];
	var id = params[1];
...
/* ----- Printers ----- */
DnaRedBullet.prototype.printBullet = function(id)
{
